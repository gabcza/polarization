#---------------------------------------------------------------------------------------------------------------------
#---- Info -----
# Goal: clean and prep of the data from polarization study 1
# 
#---------------------------------------------------------------------------------------------------------------------
#---- Load packages ----
#library(tidyverse)
...
#---- Load data ----
data <- read_sav("GRIEG+Polarization+study+1_April+11,+2024_19.00.sav") 
#summary(data) 
nrow(data) 
glimpse(data) #is like a transposed version of print()

#---- Get codebook [RUN ONCE] ----
# just run once 
...
#write.csv2(data.cdbk, "polarization study 1 codebook.csv", 
...
#------ rescale position and norms between vars 0-1 ----
...
#---- Save full data ----
write.csv2(data, "polarization study 1 full data.csv", row.names = FALSE)
#write.csv(data, "polarization study 1 full data.csv", row.names = FALSE)
...
nrow(data) # N = 1385
# att2
data <- data %>% filter(tolower(att.memory2) == "kot") 
nrow(data) # N = 1319
# att3
data <- data %>% filter(att.check.birthday == 2) 
nrow(data) # N = 1273

# remove people who didn't finish (?) - everyone finished
#data1 <- data %>% filter(!is.na(household)) 

#---- Save clean data ----
write.csv2(data, "polarization study 1 clean data.csv", row.names = FALSE)
#write.csv(data, "polarization study 1 clean data.csv", row.names = FALSE)
...
#---- Save long data ----
write.csv2(data.long, "polarization study 1 long data.csv", row.names = FALSE)
#write.csv(data.long, "polarization study 1 long data.csv", row.names = FALSE)
write.csv2(issue.means, "polarization study 1 issue means.csv", row.names = FALSE)
